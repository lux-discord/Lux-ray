...
class Admin(GeneralCog):
    ...
    async def set_lang(self, ctx, lang_code: str):
        ...
        if lang_code not in GLOBAL_SUPPORT_LANGUAGE:
            return await server.send_error(
                ctx, "Language code `{lang_code}` is not support", lang_code=lang_code
            )
        ...
        await server.send_info(
            ctx, "Successful set language to `{lang_code}`", lang_code=lang_code
        )
    ...
    async def auto_role(self, ctx, *roles: Role):
        ...
        await self.send_info(
            ctx,
            "Successful set auto-role to {roles}",
